...
const Lyrics = () =>{
    ...
    useEffect(() => {
        axios.get(`https://cors-anywhere.herokuapp.com/https://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${paramsId}&apikey=${process.env.REACT_APP_MM_KEY}`)
       .then(res => {
        console.log(res) ;
        let lyrics = res.data.message.body.lyrics; 
        setLyrics({lyrics}) ;

        return axios.get(`https://cors-anywhere.herokuapp.com/https://api.musixmatch.com/ws/1.1/track.get?track_id=${paramsId}&apikey=${process.env.REACT_APP_MM_KEY}`);
